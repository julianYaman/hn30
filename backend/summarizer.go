...
func generateSummary(articleText string) (SummaryResponse, error) {
	LogComponent("AI", "Generating summary using OpenRouter")
	...
	if err != nil {
		LogComponent("AI", "Failed to marshal OpenRouter request: %v", err)
	...
	if err != nil {
		LogComponent("AI", "OpenRouter request error: %v", err)
		return SummaryResponse{}, fmt.Errorf("AI Provider could not generate summary.")
	}
	defer res.Body.Close()

	if res.StatusCode != http.StatusOK {
		LogComponent("AI", "OpenRouter returned non-200 status: %d", res.StatusCode)
		bodyBytes, _ := io.ReadAll(res.Body)
		LogComponent("AI", "OpenRouter response body: %s", string(bodyBytes))
		return SummaryResponse{}, fmt.Errorf("AI Provider could not generate summary.")
	}
